...
/* ===== 6-cell game grid (2 rows x 3 cols) ===== */
.gameStage{
  border-radius: calc(var(--radius) + 6px);
  border: 1px solid rgba(191,232,255,.18);
  background: linear-gradient(180deg, rgba(10,16,34,.58), rgba(10,16,34,.30));
  box-shadow: 0 0 0 1px rgba(95,225,255,.08) inset, 0 18px 60px rgba(0,0,0,.55);
  overflow:hidden;
  padding: 12px;
  position:relative;
}
...
.panel{
  ...
  overflow:hidden;
  box-shadow: 0 0 0 1px rgba(95,225,255,.08) inset, 0 18px 40px rgba(0,0,0,.35);
  display:flex;
...
/* ===== Main board square (no scrollbars) ===== */
.boardCell{
  display:flex;
  flex-direction:column;
  height: 100%;
...
/* One row of clouds, even rows offset */
.tileRow{
  display:flex;
  gap: var(--tileGap);
  align-items:center;
...
/* Labels: always 2 lines, white with black outline, no label background */
.cloudLabel{
  position:relative;
  z-index: 2;
  display:flex;
...
/* Not reachable: dim + no hover motion */
.cloud.notReach{
  opacity: .58;
...
/* fog/missing/blocked are subtle, no extra shapes */
...
`;
  ...
  async function loadModeContent(nextMode: Mode) {
    ...
    scenarioIndex = Math.max(
      ...
      scenarios.findIndex(
        (s: any) =>
          String((s as any).id ?? "") === initialBase || String((s as any).name ?? "") === initialBase
      )
    );
  }
  ...
  function renderStart() {
    ...
    const p = el("div", "hint");
    p.textContent = "Start → select scenario → select player → select mode → game.";
    ...
    hero.innerHTML = `
      <img src="${toPublicUrl(START_BG_URL)}" alt="start background" onerror="this.style.display='none'"/>
  ...
  function renderSelect() {
    ...
    details.innerHTML = `
      ...
      <div class="hint" style="margin-top:10px;">
        Mode: <b>${escapeHtml(String(mode ?? "—"))}</b>
      </div>
    `;
  ...
  // --------------------------
  // Screen 3: Setup (player + optional monsters)
  // --------------------------
  ...
  function renderSetup() {
    ...
    let customPlayerImage: string | null =
  ...
  // UI-only minimap shifting (keeps your “illusion”)
  function applyMiniShiftsForEndTurn() {
    const layers = getScenarioLayerCount();
  ...
  function startScenario(idx: number) {
    ...
    // Logic reveal is allowed, but visuals do NOT depend on it (so it always looks the same)
  ...
  function renderGameScreen() {
    ...
    pMsgBody.innerHTML = `
      ...
      <div class="logSmall">(Logs every click. Rejected clicks are marked.)</div>
    `;
    pMsg.append(pMsgHead, pMsgBody);

    // 2) Top-center: Layers / Mini (Current)
    ...
    // 4) Bottom-left: Layers / Mini (Below)
    ...
    // 6) Bottom-right: Layers / Mini (Above)
    ...
    // add panels in the requested order
    ...
    // ===== Board square sizing + stable fit =====
    ...
    function renderMessage() {
      ...
      const stuckHint =
        layerReachable === 0 ? " No legal moves on this layer. Try another layer (or reset)." : "";
    ...
    function renderMiniBoardGeneric(opts: {
      ...
      if (!valid) {
        ...
        noteEl.textContent = "No tiles on this side.";
        for (let r = 1; r <= ROW_LENS.length; r++) {
          const len = ROW_LENS[r - 1] ?? 7;
      ...
      noteEl.textContent = opts.showPlayer ? "Green = your tile (this layer)." : "Structure only.";
    ...
    function renderBoard() {
      ...
      for (let r = 1; r <= ROW_LENS.length; r++) {
        ...
        for (let c = 1; c <= len; c++) {
          ...
          // IMPORTANT: visuals should not suddenly change after "force reveal"
          // So we keep fog class for unrevealed, but it only changes opacity subtly (not shapes).
    ...
    function setLayerOptions(layerSel: HTMLSelectElement) {
      const layers = getScenarioLayerCount();
      layerSel.innerHTML = "";
      ...
      layerSel.value = String(currentLayer);
    }
    ...
    forceRevealBtn.addEventListener("click", () => {
      // Logic reveal remains; visuals stay consistent (no shape swap)
      revealWholeLayer(currentLayer);
      recomputeReachability();
